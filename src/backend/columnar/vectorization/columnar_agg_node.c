...
static TupleTableSlot *agg_retrieve_direct(VectorAggState *aggstate);
static void agg_fill_hash_table(VectorAggState *aggstate);
static bool agg_refill_hash_table(AggState *aggstate);
static TupleTableSlot *agg_retrieve_hash_table(VectorAggState *aggstate);
static TupleTableSlot *agg_retrieve_hash_table_in_memory(VectorAggState *vas);
		...
		switch (node->phase->aggstrategy)
		{
			case AGG_HASHED:
				if (!node->table_filled)
					agg_fill_hash_table(vectoraggstate); // HYDRA: need work with this
				/* FALLTHROUGH */
			case AGG_MIXED:
				result = agg_retrieve_hash_table(vectoraggstate); // HYDRA need work with this
				break;
			case AGG_PLAIN:
			case AGG_SORTED:
	...
	while (!aggstate->agg_done)
	{
			...
			aggstate->projected_set >= (numGroupingSets - 1))
		{
			...
			else if (aggstate->aggstrategy == AGG_MIXED)
			{
				...
				return agg_retrieve_hash_table(vectoraggstate);
			}
			else
			{
		...
		vslot = (VectorTupleSlot *)vectoraggstate->vectorResultSlot;
		
		vector_clear_slot((TupleTableSlot *)vslot);
 ...
 */
static void
agg_fill_hash_table(VectorAggState *vectoraggstate)
{
	TupleTableSlot *outerslot;
	AggState *aggstate = vectoraggstate->aggstate;
	ExprContext *tmpcontext = aggstate->tmpcontext;
 ...
 */
static TupleTableSlot *
agg_retrieve_hash_table(VectorAggState *vectoraggstate)
{
	TupleTableSlot *result = NULL;
	AggState *aggstate = vectoraggstate->aggstate;
	...
	while (result == NULL)
	{
		result = agg_retrieve_hash_table_in_memory(vectoraggstate);
		if (result == NULL)
		{
 ...
 */
static TupleTableSlot *
agg_retrieve_hash_table_in_memory(VectorAggState *vas)
{
	...
	vslot = (VectorTupleSlot *)vas->vectorResultSlot;
...
BeginVectorAgg(CustomScanState *css, EState *estate, int eflags)
{
	...
	Agg	*node = (Agg *)linitial(cscan->custom_plans);
	...
	/* Vector tuple slot initialization */
	vector_init_result_slot(estate, &vas->css.ss.ps);
...
{
	VectorTupleSlot	*vectorSlot;
	TupleDesc tupleDesc;
	int	i;

	tupleDesc = vas->aggstate->ss.ps.ps_ResultTupleSlot->tts_tupleDescriptor;

	vas->vectorResultSlot = vector_init_extra_slot(estate, tupleDesc);

	vectorSlot = (VectorTupleSlot *)vas->vectorResultSlot;

	/* initailize tuple batch */
	for (i = 0; i < tupleDesc->natts; i++)
	{
		Oid typid = TupleDescAttr(vas->vectorResultSlot->tts_tupleDescriptor, i)->atttypid;
		int16 typlen = TupleDescAttr(vas->vectorResultSlot->tts_tupleDescriptor, i)->attlen;
		Vtype *column = build_vtype(typid, typlen, VECTOR_BATCH_SIZE, vectorSlot->skip);
		vas->vectorResultSlot->tts_values[i] = PointerGetDatum(column);
		vas->vectorResultSlot->tts_isnull[i] = false;
	}
}
...
void
columnar_agg_node_register(void)
{
